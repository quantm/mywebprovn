...
class luanvan_model extends CI_Model {

	function SelectAll($limit, $offset, $sort_by, $sort_order) {

		//search
		$data = array('NAME' => $this->input->get_post('search'), 'ID_CATEGORY'=>$this->input->get_post('id_category'),'ID_SUB_CATEGORY'=>$this->input->get_post('id_sub_category'));
        ...
        // results query
        $q = $free_db->select('*')->from('ebook_index')->limit($limit, $offset)->order_by($sort_by, $sort_order);
		...
		if($r){
			//count
			 $count = $free_db->select('COUNT(*) as count', FALSE)->from('ebook_index')->where('REFERER','luanvannetvn');
			if (isset($_REQUEST['search'])) {
			...
			if (isset($_REQUEST['id_sub_category'])) {
			 ...
			 if (isset($_REQUEST['id_category'])) {
				redirect('http://www.xahoihoctap.net.vn/luanvan/index?id_category='.$_REQUEST['id_category']);
			 }
			 else {
				redirect('http://www.xahoihoctap.net.vn/luanvan/');
			 }
		}  
        $ret['rows'] = $r;
