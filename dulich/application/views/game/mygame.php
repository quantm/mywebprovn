...
<ul class="breadcrumb">
	<li class="active">
	<a class="font-effect-outline" href="/ebook/">Tất cả game<a/>
...
<?php $condition = empty($mygame) || !is_array($mygame) ? 0 : count($mygame); ?>
<?php if ($condition) {
$loop = -1;
foreach ($mygame as $key) {                    
$loop++; ?>
...
<span><?= $key['NAME'] ?></span>
<a href="http://myweb.pro.vn/play?id=<?= $key['ID'] ?>" target="_new"><button id="ebook_item_<?= $key['ID'] ?>"  data-href="http://myweb.pro.vn/play?id=<?= $key['ID'] ?>" data-name="<?= $key['NAME'] ?>" class="game_list btn btn-primary" >Chơi ngay</button></a>
</div>
<?php } ?>
</td>
<?php }
