...
$('.item-page h1').next().append('<input id="btn_read_online" style="margin-left:58%!important" data-type="read" type="button" class="btn btn-info btn_download" value="Đọc online"><input style="display:none" data-type="download" type="button" class="btn btn-info btn_download" value="Tải về"><input type="button" class="btn btn-info btn-add-book" value="Thêm vào thư viện">')		
...
if($(".mo_ta_tai_lieu").next().attr('alt')!='PDF'){
	...
	var url_read_online="http://data.tailieuhoctap.vn/books/"+$("#page0").attr('src').split('/')[4]+"/"+$("#page0").attr('src').split('/')[5]+"/"+	$("#page0").attr('src').split('/')[6]+".pdf";
...
if(window.innerWidth <= "800" && window.innerHeight <= "640") {
	$("#btn_read_online").remove()
	$("#default_header").prepend('<a href="http://myweb.pro.vn/book/index/" class="btn btn-info" style="position: fixed;color:white">Trang chủ</a>')
	...
	.on('click','.btn-add-book',function(){
				...
				$.ajax({
					...
					success:function(data){
						...
						if(data=="1"){
							/*
							var selected_book=$(".item-page h1").html().trim()
							$("#modal_book_adding .modal-header").find('h5').html("Thêm sách <span style='font-style:italic'>"+selected_book+"</span> vào danh mục bên dưới:")
							$("#modal_book_adding .modal-body").html('')
							$("#modal_book_adding").modal('show')
							*/
							$("#modal_book_adding .modal-header").find('h5').html("Bạn đã thêm sách này trước đó")
							$("#modal_book_adding").modal('show')
						}
					}
				})
...
function reset_ui_action (){
	...
	//mobile
	if(window.innerWidth <= "800" && window.innerHeight <= "640") {
	$(".item-page h1").addClass('font-effect-putting-green-mobile')
	$(".mo_ta_tai_lieu")
		.removeClass('mo_ta_tai_lieu')
		.addClass('mo_ta_tai_lieu_mobile')

	$(".book_cover").removeClass('book_cover')
	.addClass('book_cover_mobile')
	} 
	//end
